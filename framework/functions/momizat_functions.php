...
function mom_post_image($size = 'thumbnail', $id = ''){
		...
		if ($tt == 1) {
			if ($size == 'full') {
				...
				if ($image) return MOM_URI.'/framework/timthumb/timthumb.php?src='.$image.'&h='.$h.'&w='.$w.'&zc=3';
			}
		} else {
			if ($image) return $image;
		}
		...
		if (mom_option('post_first_image') == 1) {
					if (mom_get_first_image($id)) {
						if ($tt == 1) {
	                            return MOM_URI.'/framework/timthumb/timthumb.php?src='.mom_get_first_image($id).'&h='.$h.'&w='.$w.'&zc=2';
						} else {
...
?>
            <div class="np-posts">
                <ul>
					<li class="prev">Bài trước:</br><?php previous_post_link('%link','%title',TRUE); ?></li>
					
					<li>Bài kế tiếp:</br><?php next_post_link('%link','%title',TRUE); ?></li>
                </ul>
            </div> <!-- np posts -->
			<?php
...
if (! function_exists('mom_posts_meta')) {
	...
	if($tags){
		...
		$date = mom_option('post_meta-date') == 1 ? '<span>'.__('Ngày đăng:', 'theme').' <time datetime="'.get_the_time('c').'" class="updated">'.get_mom_date_format().'</time></span>': '';
		...
		$cat = mom_option('post_meta-cat') == 1 ? '<span>'.__('Mục', 'theme').': '.trim($cats, $separator).'</span>': '';
...
/* ==========================================================================
                 	Su kien
========================================================================== */
function mom_related_sukien ($tag) { 
global $post;
?>
  <h2 class="single-title"><?php _e('Các bài khác:', 'theme'); ?></h2>
...
<?php } elseif ($relatedby == 'category' || $relatedby == 'both' ) { ?>
		...
		if ($cats) :
			...
			'post__not_in' => array($post->ID,$next_post->ID,$prev_post->ID),
			'posts_per_page'=>$count,
			'ignore_sticky_posts'=>1
		    );
		query_posts($args);
...
<?php endwhile; ?>
<?php  else:  ?>
<h5><?php echo __('Không có bài viết nào trong mục này', 'theme'); ?></h5>
<?php  endif; ?>
<?php wp_reset_query(); ?>
<?php endif;?>
<?php } ?>
...
if (! function_exists('mom_logo')) { 
	...
	?>
		    ...
		    ?>
                    ...
                    <?php } ?>
                    <?php if($r_logo != '') { ?>
                        <img class="mom_retina_logo" src="<?php echo $r_logo; ?>"  alt="<?php bloginfo('name'); ?>" />
                        <?php } else { ?>
                           <?php if ($the_logo != ''){ ?>
