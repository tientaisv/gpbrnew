	...
	<div class="fslides fs-<?php echo $rndn; ?>">
		...
		?>
                        <div class="row">
							...
							<div class="col-md-6 featured-thumbnails">
...
global $la;
	...
	?>
                ...
                <div class="thi-news-list <?php echo $class ?>">
                    <header>
	                ...
	                <?php     $random_query = new WP_Query(array(
				        ...
				        if ( $random_query->have_posts() ) :
								...
								<?php
								$dong1 = get_post_meta(get_the_id(),'mom_field1');
								$dong2 = get_post_meta(get_the_id(),'mom_field2');
								$nen = get_post_meta(get_the_id(),'mom_bg');
								if($nen[0] != ''){
									$nen = $nen[0];
								...
								?>
				                            <div id="youcat" style="background:<?php echo $nen; ?>">
											<?php if($dong1[0] != ''){ ?>
												<div class="dong1"><?php echo $dong1[0] ; ?></div>
												<?php }  if($dong2[0] != ''){?>
												<div class="dong2"><?php echo $dong2[0] ; ?></div>
											<?php	} ?>
											</div>
		...
		<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); if ($unique_posts) {$do_unique_posts[] = get_the_ID();} ?>
                            <article>
                                <div class="news-list-image">
                                        <a href="<?php the_permalink(); ?>"><?php mom_post_image_full('news_list_img', 'news_list_img_hd'); ?></a>
                                </div>
                                   <h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
                            </article>
		<?php endwhile; else: ?>
...
<?php } elseif ($style == 3) { ?>
		...
		<!--News box gpbaria-->
        <div class="thi-news-box">
				<?php
					$sticky = get_option( 'sticky_posts' );
					$args = array(
					'cat' => $category,
					'ignore_sticky_posts' => 1,
					'posts_per_page' => 1, 'post_type' => $post_type, 'category__not_in' => $exclude_categories, 'cache_results' => false, 'no_found_rows' => true, 'post__not_in' => $sticky,
					'orderby' => $orderby,
					'order' => $sort,
					);
				$query = new WP_Query( $args );
			...
			<header class="nb-header">
				...
				<nav class="navbar navbar-default">
					<div class="navbar-header">
					  <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#cat-<?php echo $category ?>" aria-expanded="false">
					...
					<div class="collapse navbar-collapse" id="cat-<?php echo $category ?>">
		...
		<?php wp_reset_postdata(); ?>
                            ...
                            <div class="col-md-3 other-articles">
                                <ul>
									<?php
										$sticky = get_option( 'sticky_posts' );
										$args = array(
										'cat' => $category,
										'ignore_sticky_posts' => 1,
										'posts_per_page' => 2, 'post_type' => $post_type, 'category__not_in' => $exclude_categories, 'cache_results' => false, 'no_found_rows' => true, 'post__not_in' => $sticky,
										'orderby' => $orderby,
										'order' => $sort,
										'offset'=>'1');
									...
									<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); if ($unique_posts) {$do_unique_posts[] = get_the_ID();} ?>
											...
											if (mom_post_image() != false) {
												$is_img = 'has-feature-image';
											?>
											 <a href="<?php the_permalink(); ?>"><?php mom_post_image_full('news_box3', 'big-wide-img'); ?></a>
											<?php } ?>
																	<div class="details <?php echo $is_img; ?>">
											<h4><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
															   </div>
							...
							<div class="col-md-4 other-articles2">
                                <ul>
									<?php
										$sticky = get_option( 'sticky_posts' );
										$args = array(
										'cat' => $category,
										'ignore_sticky_posts' => 1,
										'posts_per_page' => $count, 'post_type' => $post_type, 'category__not_in' => $exclude_categories, 'cache_results' => false, 'no_found_rows' => true, 'post__not_in' => $sticky,
										'orderby' => $orderby,
										'order' => $sort,
										'offset'=>'3');
...
<?php
function create_shortcode_lccn() {
        ...
        if ( $random_query->have_posts() ) :
                ...
                while ( $random_query->have_posts() ) :
                        $random_query->the_post();?>
				<?php
				$loichua = get_post_meta(get_the_id(),''); ?>
                            <div id="songloichuahangtuan">
								<div class="title"><?php the_title(); ?></div>
								<div class="title-giaoly" style="margin-top:10px;">CẦU NGUYỆN VỚI LỜI CHÚA HÀNG TUẦN</div>
								<div class="loichua"><?php echo $loichua['lccn_trichdoan'][0] ; ?></div>
								<div class="loinguyen"><?php echo $loichua['lccn_loinguyen'][0] ; ?></div>
								<div class="title-giaoly" style="margin-top:10px;">HỌC HỎI GIÁO LÝ HỘI THÁNH CÔNG GIÁO</div>
								<div class="cau"><?php echo $loichua['lccn_cau'][0] ; ?></div>
								<div class="loinguyen"><?php echo $loichua['lccn_thua'][0] ; ?></div>
							</div>
...
add_shortcode('lccn', 'create_shortcode_lccn');
// Shortcode Tweet
function create_shortcode_tweet() {
        ...
        if ( $random_query->have_posts() ) :
                ...
                while ( $random_query->have_posts() ) :
                        $random_query->the_post();?>
						<?php
								$content = get_post_meta(get_the_id(),'mom_content');
								$nen = get_post_meta(get_the_id(),'mom_bg');
								if($nen[0] != ''){
									$nen = $nen[0];
				...
				<?php
						if($content[0] != '') {
                            ...
                            <div id="pope-tweet" style="background:<?php echo $nen; ?>; color:#2c5fb0">
								<div class="tweet-content"><?php echo $content[0] ; ?></div>
							</div>
...
add_shortcode('tweet', 'create_shortcode_tweet');
?>
<?php
